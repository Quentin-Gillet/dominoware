...
namespace FGUI
{
	...
	using COLOR = struct SColor_t
	{
		...
		SColor_t(std::uint8_t red, std::uint8_t green, std::uint8_t blue, float alpha) : m_ucRed(red), m_ucGreen(green), m_ucBlue(blue)
		{
			m_ucAlpha = static_cast<std::uint8_t>(alpha);
		}
		SColor_t(float red, float green, float blue, std::uint8_t alpha)
		{
			m_ucRed = static_cast<std::uint8_t>(red);
			m_ucGreen = static_cast<std::uint8_t>(green);
			m_ucBlue = static_cast<std::uint8_t>(blue);
			m_ucAlpha = alpha;
		}
		SColor_t(float red, float green, float blue, float alpha)
		{
			m_ucRed = static_cast<std::uint8_t>(red);
			m_ucGreen = static_cast<std::uint8_t>(green);
			m_ucBlue = static_cast<std::uint8_t>(blue);
		...
		SColor_t(int red, int green, int blue, int alpha) : m_ucRed(red), m_ucGreen(green), m_ucBlue(blue), m_ucAlpha(alpha) {}
		...
		static SColor_t HSBToRGB(float hue, float saturation, float brightness, std::uint8_t alpha = 255)
		{
			...
			if (flHue < 1.f)
			{
				return SColor_t((brightness * 255.f), (flT * 255.f), (flP * 255.f), alpha);
			}
			else if (flHue < 2.f)
			{
				return SColor_t((flQ * 255.f), (brightness * 255.f), (flP * 255.f), alpha);
			}
			else if (flHue < 3.f)
			{
				return SColor_t((flP * 255.f), (brightness * 255.f), (flT * 255.f), alpha);
			}
			else if (flHue < 4.f)
			{
				return SColor_t((flP * 255.f), (flQ * 255.f), (brightness * 255.f), alpha);
			}
			else if (flHue < 5.f)
			{
				return SColor_t((flT * 255.f), (flP * 255.f), (brightness * 255.f), alpha);
			}
			else
			{
				return SColor_t((brightness * 255.f), (flP * 255.f), (flQ * 255.f), alpha);
			}
		}

...
namespace FGUI
{
	...
	void CListBox::Geometry(FGUI::WIDGET_STATUS status)
	{
		...
		// listbox body
		FGUI::RENDER.Outline(arWidgetRegion.m_iLeft, arWidgetRegion.m_iTop + (iEntriesDisplayed * m_iEntrySpacing), m_dmSize.m_iWidth, m_dmSize.m_iHeight, { 50, 50, 50 });
		FGUI::RENDER.Rectangle((arWidgetRegion.m_iLeft + 1), (arWidgetRegion.m_iTop + (iEntriesDisplayed * m_iEntrySpacing)) + 1, (m_dmSize.m_iWidth - 2), (m_dmSize.m_iHeight - 2), { 40, 40, 40 });
		...
		for (std::size_t i = m_iScrollThumbPosition; (i < m_prgpEntries.first.size()) && (iEntriesDisplayed < iCalculatedEntries); i++)
		{
			...
			if (FGUI::INPUT.IsCursorInArea(arEntryRegion) || m_ullSelectedEntry == i)
			{
				...
				FGUI::RENDER.Text((arEntryRegion.m_iLeft + 5), (arEntryRegion.m_iTop + 3), m_anyFont, accentColor, m_prgpEntries.first[i]);
			}
			else
			{
				//FGUI::RENDER.Rectangle(arEntryRegion.m_iLeft, (arEntryRegion.m_iTop + arEntryRegion.m_iBottom) - 1, arEntryRegion.m_iRight, 1, { 205, 205, 205 });
				FGUI::RENDER.Text((arEntryRegion.m_iLeft + 5), (arEntryRegion.m_iTop + 3), m_anyFont, {255, 255, 255}, m_prgpEntries.first[i]);
			}
		...
		{
			FGUI::RENDER.Rectangle((arScrollBarRegion.m_iLeft + 4), (arScrollBarRegion.m_iTop + flCalculatedPosition) + 5, dmScrollBarThumb.m_iWidth, dmScrollBarThumb.m_iHeight, { 60, 60, 60 });
		}
		else
		{
			FGUI::RENDER.Rectangle((arScrollBarRegion.m_iLeft + 4), (arScrollBarRegion.m_iTop + flCalculatedPosition) + 5, dmScrollBarThumb.m_iWidth, flCalculatedSize, { 60, 60, 60 });
		}

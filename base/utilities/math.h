...
namespace M
{
	...
	void	RotatePoint(const Vector2D& vecIn, const float flAngle, Vector2D* pOutPoint);
	/* rotates given center point and outputs to given outpoint by given angle */
	void	RotateCenter(const Vector2D& vecCenter, const float flAngle, Vector2D* pOutPoint);

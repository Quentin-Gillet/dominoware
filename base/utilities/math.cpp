...
void M::RotatePoint(const Vector2D& vecIn, const float flAngle, Vector2D* pOutPoint)
{
	if (&vecIn == pOutPoint)
	{
		const Vector2D vecPoint = vecIn;
...
void M::RotateCenter(const Vector2D& vecCenter, const float flAngle, Vector2D* pOutPoint)
{
	const float flSin = std::sinf(M_DEG2RAD(flAngle));
	const float flCos = std::cosf(M_DEG2RAD(flAngle));

...
#pragma region visuals_store_world
	...
	//		D::AddLine(Vector2D(0.f, y * 0.5f), Vector2D(x, y * 0.5f), Color(0, 0, 0, 200));
	//		D::AddLine(Vector2D(x * 0.5f, 0.f), Vector2D(x * 0.5f, y), Color(0, 0, 0, 200));
...
#pragma region visuals_store_screen
	/*if (C::Get<bool>(Vars.bScreen))
	{
		if (C::Get<bool>(Vars.bScreenHitMarker))
			HitMarker(Vector2D(x, y), flServerTime, C::Get<Color>(Vars.colScreenHitMarker), C::Get<Color>(Vars.colScreenHitMarkerDamage));
	}*/
#pragma endregion
...
void CVisuals::HitMarker(const Vector2D& vecScreenSize, float flServerTime, Color colLines, Color colDamage)
{
	...
	std::erase_if(vecHitMarks, [&](const auto& hitmarkObject)
		{
			...
			//if (Vector2D vecScreen = { }; D::WorldToScreen(hitmarkObject.vecPosition, vecScreen))
			//{
			...
			//	// draw dealt damage
			//	D::AddText(Vector2D(vecScreen.x, vecScreen.y - flRatio * flDistance), F::Whitney, std::to_string(hitmarkObject.iDamage), colDamage.Set<COLOR_A>(static_cast<std::uint8_t>(iAlpha)));
			//}
...
void CVisuals::Bomb(const Vector2D& vecScreen, Context_t& ctx, const Color& colFrame) const
{
}

void CVisuals::PlantedBomb(CPlantedC4* pBomb, float flServerTime, const Vector2D& vecScreen, Context_t& ctx, const Color& colFrame, const Color& colDefuse, const Color& colFailDefuse, const Color& colBackground, const Color& colOutline) const
{
	
}

void CVisuals::Grenade(CBaseEntity* pGrenade, EClassIndex nIndex, float flServerTime, const Vector2D& vecScreen, Context_t& ctx, const Color& colFrame, const Color& colBackground, const Color& colOutline) const
{
	...
	//const Vector2D vecNameSize = D::GetTextSize(F::Whitney, szName);
	//const Vector2D vecSize(vecNameSize.x + 10.f, vecNameSize.y + 6.f);
	...
	///* info */
	//// frame
	//D::AddFilledRect(Vector2D(ctx.box.left - 1, ctx.box.top), Vector2D(ctx.box.right + 1, ctx.box.bottom), colFrame);
	//// text
	//D::AddText(Vector2D(ctx.box.left + 5, ctx.box.top + 3), F::Whitney, szName, Color(255, 255, 255));

	//if (flFactor > 0.f)
	//{
	//	/* bar */
	//	// background
	//	D::AddFilledRect(Vector2D(ctx.box.left, ctx.box.bottom + 2), Vector2D(ctx.box.right, ctx.box.bottom + 4), colBackground);
	//	// bar
	//	D::AddFilledRect(Vector2D(ctx.box.left, ctx.box.bottom + 2), Vector2D(ctx.box.left + ctx.box.width * flFactor, ctx.box.bottom + 4), colGrenade);
	//}
}
...
void CVisuals::DroppedWeapons(CBaseCombatWeapon* pWeapon, short nItemDefinitionIndex, Context_t& ctx, const float flDistance, const Color& colPrimary, const Color& colAmmo, const Color& colBackground, const Color& colOutline)
{
	...
	//	const Vector2D vecNameSize = D::GetTextSize(F::Icons, szIcon);

	//	D::AddText(Vector2D(ctx.box.left + ctx.box.width * 0.5f - vecNameSize.x * 0.5f, ctx.box.bottom + 3), F::Icons, szIcon, colPrimary);
	...
	//	const Vector2D vecDistanceSize = D::GetTextSize(F::Whitney, szDistance.c_str());

	//	D::AddText(Vector2D(ctx.box.left + ctx.box.width * 0.5f - vecDistanceSize.x * 0.5f, ctx.box.bottom + 2 + ctx.arrPadding.at(DIR_BOTTOM)), F::Whitney, szDistance, colPrimary);
...
void CVisuals::Player(CBaseEntity* pLocal, CBaseEntity* pEntity, Context_t& ctx, const float flDistance, const Color& colInfo, const Color& colFrame, const Color& colOutline)
{
	...
	//	Box(ctx.box, 0, colBox, Color(0, 0, 0, 150));
	//}
	...
	//	Vector2D vecBotSize{ };
	//	const Vector2D vecNameSize = D::GetTextSize(F::Whitney, szName.c_str());

	//	// add prefix for bots
	//	if (playerInfo.bFakePlayer)
	//	{
	//		vecBotSize = D::GetTextSize(F::Whitney, szBot);
	//		D::AddText(Vector2D(ctx.box.left + ctx.box.width * 0.5f + 1 + vecNameSize.x * 0.5f - vecBotSize.x * 0.5f, ctx.box.top - 2 - vecBotSize.y - ctx.arrPadding.at(DIR_TOP)), F::Whitney, szBot, Color(140, 140, 140));
	//	}

	//	D::AddText(Vector2D(ctx.box.left + ctx.box.width * 0.5f - vecNameSize.x * 0.5f - vecBotSize.x * 0.5f, ctx.box.top - 2 - vecNameSize.y - ctx.arrPadding.at(DIR_TOP)), F::Whitney, szName, colInfo);
	...
	if (CBaseCombatWeapon* pActiveWeapon = pEntity->GetWeapon(); pActiveWeapon != nullptr)
	{
		...
		//			const Vector2D vecIconSize = D::GetTextSize(F::Icons, szIcon);
		//			D::AddText(Vector2D(ctx.box.left + ctx.box.width * 0.5f - vecIconSize.x * 0.5f, ctx.box.bottom + 2 + ctx.arrPadding.at(DIR_BOTTOM)), F::Icons, szIcon, pCurrentWeapon == pActiveWeapon ? colInfo : Color(160, 160, 160));
	...
	/*if (C::Get<bool>(Vars.bEspMainPlayerDistance))
	{
		...
		std::string szDistance = std::to_string(iDistance).append(XorStr("M"));
		const Vector2D vecDistanceSize = D::GetTextSize(F::Whitney, szDistance.c_str());
		D::AddText(Vector2D(ctx.box.left + ctx.box.width * 0.5f - vecDistanceSize.x * 0.5f, ctx.box.bottom + 2 + ctx.arrPadding.at(DIR_BOTTOM)), F::Whitney, szDistance, colInfo);
		ctx.arrPadding.at(DIR_BOTTOM) += vecDistanceSize.y;
	}*/
	#pragma endregion
	...
	//	const Vector2D vecMoneySize = D::GetTextSize(F::Whitney, szMoney);
	//	D::AddText(Vector2D(ctx.box.left - 2 - vecMoneySize.x - ctx.arrPadding.at(DIR_LEFT), ctx.box.top), F::Whitney, szMoney, Color(140, 195, 75));
	...
	/*if (C::Get<std::vector<bool>>(Vars.vecEspMainPlayerFlags).at(INFO_FLAG_HELMET) && pEntity->HasHelmet())
	{
		...
		const Vector2D vecHelmetSize = D::GetTextSize(F::Icons, szHelmetIcon);
		D::AddText(Vector2D(ctx.box.right + 2, ctx.box.top + ctx.arrPadding.at(DIR_RIGHT)), F::Icons, szHelmetIcon, colInfo);
	...
	if (C::Get<std::vector<bool>>(Vars.vecEspMainPlayerFlags).at(INFO_FLAG_KEVLAR) && pEntity->GetArmor() > 0)
	{
		...
		const Vector2D vecKevlarSize = D::GetTextSize(F::Icons, szKevlarIcon);
		D::AddText(Vector2D(ctx.box.right + 2, ctx.box.top + ctx.arrPadding.at(DIR_RIGHT)), F::Icons,  szKevlarIcon, colInfo);
	...
	if (C::Get<std::vector<bool>>(Vars.vecEspMainPlayerFlags).at(INFO_FLAG_KIT) && pEntity->HasDefuser())
	{
		...
		const Vector2D vecKitSize = D::GetTextSize(F::Icons, szKitIcon);
		D::AddText(Vector2D(ctx.box.right + 2, ctx.box.top + ctx.arrPadding.at(DIR_RIGHT)), F::Icons, szKitIcon, pEntity->IsDefusing() ? Color(80, 180, 200) : colInfo);
	...
	if (C::Get<std::vector<bool>>(Vars.vecEspMainPlayerFlags).at(INFO_FLAG_ZOOM) && pEntity->IsScoped())
	{
		...
		const Vector2D vecZoomSize = D::GetTextSize(F::Icons, szTargetIcon);
		D::AddText(Vector2D(ctx.box.right + 2, ctx.box.top + ctx.arrPadding.at(DIR_RIGHT)), F::Icons, szTargetIcon, colInfo);
...
void CVisuals::Box(const Box_t& box, const int nBoxType, const Color& colPrimary, const Color& colOutline)
{
	...
	//{
	//	D::AddRect(Vector2D(box.left, box.top), Vector2D(box.right, box.bottom), colPrimary);
	//	break;
	//}
	...
	//	const std::pair<Vector2D, Vector2D> arrPoints[] =
	//	{
	//		// left vertical lines
	//		{ Vector2D(box.left, box.top), Vector2D(box.left, box.top + box.height / nDivideParts) },
	//		{ Vector2D(box.left, box.bottom), Vector2D(box.left, box.bottom - box.height / nDivideParts) },

	//		// right vertical lines
	//		{ Vector2D(box.right, box.top), Vector2D(box.right, box.top + box.height / nDivideParts) },
	//		{ Vector2D(box.right, box.bottom), Vector2D(box.right, box.bottom - box.height / nDivideParts) },

	//		// left horizontal lines
	//		{ Vector2D(box.left, box.top), Vector2D(box.left + box.width / nDivideParts, box.top) },
	//		{ Vector2D(box.left, box.bottom), Vector2D(box.left + box.width / nDivideParts, box.bottom) },

	//		// right horizontal lines
	//		{ Vector2D(box.right, box.top), Vector2D(box.right - box.width / nDivideParts, box.top) },
	//		{ Vector2D(box.right, box.bottom), Vector2D(box.right - box.width / nDivideParts, box.bottom) }
	//	};
...
{
	// background
	D::AddRect(Vector2D(ctx.box.left - 5 - ctx.arrPadding.at(DIR_LEFT), ctx.box.top), Vector2D(ctx.box.left - 3 - ctx.arrPadding.at(DIR_LEFT), ctx.box.bottom), colBackground);
	// bar
	D::AddRect(Vector2D(ctx.box.left - 5 - ctx.arrPadding.at(DIR_LEFT), ctx.box.bottom - (ctx.box.height * flFactor)), Vector2D(ctx.box.left - 3 - ctx.arrPadding.at(DIR_LEFT), ctx.box.bottom), colPrimary);
...
void CVisuals::AmmoBar(CBaseEntity* pEntity, CBaseCombatWeapon* pWeapon, Context_t& ctx, const Color& colPrimary, const Color& colBackground, const Color& colOutline)
{
	...
	// background
	D::AddRect(Vector2D(ctx.box.left, ctx.box.bottom + 3 + ctx.arrPadding.at(DIR_BOTTOM)), Vector2D(ctx.box.right, ctx.box.bottom + 5 + ctx.arrPadding.at(DIR_BOTTOM)), colBackground);
	// bar
	D::AddRect(Vector2D(ctx.box.left, ctx.box.bottom + 3 + ctx.arrPadding.at(DIR_BOTTOM)), Vector2D(ctx.box.left + ctx.box.width * flFactor, ctx.box.bottom + 5 + ctx.arrPadding.at(DIR_BOTTOM)), colPrimary);
...
void CVisuals::FlashBar(CBaseEntity* pEntity, Context_t& ctx, const Color& colPrimary, const Color& colBackground, const Color& colOutline)
{
	...
	// background
	D::AddRect(Vector2D(ctx.box.left, ctx.box.top - 5 - ctx.arrPadding.at(DIR_TOP)), Vector2D(ctx.box.right, ctx.box.top - 3 - ctx.arrPadding.at(DIR_TOP)), colBackground);
	// bar
	D::AddRect(Vector2D(ctx.box.left, ctx.box.top - 5 - ctx.arrPadding.at(DIR_TOP)), Vector2D(ctx.box.left + ctx.box.width * flFactor, ctx.box.top - 3 - ctx.arrPadding.at(DIR_TOP)), colPrimary);
